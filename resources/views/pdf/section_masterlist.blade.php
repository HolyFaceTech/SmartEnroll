...
<head>
    <meta charset="UTF-8">
    <title>{{ strtoupper($section->name) }} Master List</title>
    <style>
        @page { 
            margin: 30px 50px; 
        }
...
<body>
    ...
    <div class="doc-title">SECTION OFFICIAL ENROLLMENT LIST</div>

    <table class="meta-table">
        <tr>
            <td class="meta-label">SECTION:</td>
            <td class="meta-val">{{ $section->grade_level }} - {{ strtoupper($section->name) }}</td>
            <td class="meta-label">SCHOOL YEAR:</td>
            <td class="meta-val">{{ $schoolYear ?? date('Y').'-'.(date('Y')+1) }}</td>
        </tr>
        <tr>
            <td class="meta-label">STRAND:</td>
            <td class="meta-val">{{ strtoupper($section->strand->code) }}</td>
            <td class="meta-label">TOTAL ENROLLED:</td>
            <td class="meta-val">{{ count($students) }} / {{ $section->capacity }}</td>
        </tr>
    </table>

    @php
        $males = $students->where('gender', 'Male')->sortBy('last_name');
        $females = $students->where('gender', 'Female')->sortBy('last_name');
    @endphp
